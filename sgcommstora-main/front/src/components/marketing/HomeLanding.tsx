...
const benefits = [
  {
    ...
    body: "Décrivez votre niche, votre cible et l’univers souhaité. Stora transforme votre idée en identité, palette et boutique complète.",
    metric: "1 prompt",
...
const faqs = [
  ["Faut-il savoir coder pour utiliser Stora ?", "Non. Vous décrivez votre projet en langage naturel et Stora construit la boutique. Chaque élément reste aussi modifiable manuellement."],
  ...
  ["Comment les visuels produit sont-ils générés ?", "Stora utilise des modèles de génération d’images pour créer des visuels photoréalistes adaptés à votre identité de marque."],
...
function ProductVisual({ index }: { index: number }) {
  if (index === 0) {
    return (
      <div className="flex h-full flex-col justify-end gap-3 p-5 sm:p-6" aria-hidden>
        ...
        <div className="max-w-[88%] rounded-[20px] rounded-bl-md border border-black/[0.07] bg-white/65 p-4">
          ...
          <div className="flex gap-2">
            ...
            <span className="h-8 flex-1 rounded-full bg-[#86aaa0]" />
            <span className="h-8 flex-1 rounded-full bg-[#20b9bf]" />
          </div>
        </div>
      </div>
    );
  ...
  if (index === 1) {
    return (
      <div className="grid h-full grid-cols-3 items-end gap-2 p-5 sm:p-6" aria-hidden>
        <div className="translate-y-4 rounded-[18px] bg-white/70 p-2 shadow-sm">
        ...
        <div className="rounded-[18px] bg-white p-2 shadow-[0_18px_35px_-24px_rgba(0,0,0,.5)]">
          <div className="aspect-[4/5] rounded-[13px] bg-gradient-to-br from-[#a2cbc2] via-[#26c2be] to-[#278fb3]" />
        ...
        <div className="translate-y-7 rounded-[18px] bg-white/70 p-2 shadow-sm">
  ...
  return (
    <div className="grid h-full grid-cols-[1.15fr_.85fr] gap-2 p-5 sm:p-6" aria-hidden>
      <div className="relative overflow-hidden rounded-[22px] bg-[radial-gradient(circle_at_35%_25%,rgba(255,255,255,.75),transparent_18%),linear-gradient(145deg,#87afa5,#20bbb9_58%,#237fa6)]">
        <div className="absolute inset-x-[24%] bottom-[12%] top-[20%] rounded-[45%_45%_28%_28%] border border-white/35 bg-white/25 shadow-[inset_0_1px_rgba(255,255,255,.5)] backdrop-blur-sm" />
...
function DashboardShowcase() {
  ...
  return (
    <figure role="img" aria-label="Aperçu du tableau de bord de la boutique Atelier Nival" className="relative mx-auto mt-16 max-w-[1240px] px-4 sm:px-7">
      ...
      <div className="relative overflow-hidden rounded-[24px] border border-black/[0.08] bg-[#cfcfcf] p-1.5 shadow-[0_42px_95px_-58px_rgba(0,0,0,.55)] sm:rounded-[32px] sm:p-3">
        <div className="grid overflow-hidden rounded-[19px] bg-[#dfdfdf] sm:rounded-[23px] lg:grid-cols-[58px_1fr]">
          <aside className="hidden flex-col items-center gap-3 border-r border-black/5 bg-[#e7e7e7] py-5 lg:flex" aria-hidden>
          ...
          <div className="min-w-0">
            <div className="flex h-14 items-center justify-between border-b border-black/5 px-4 sm:px-6">
              <div className="flex items-center gap-3 text-xs"><strong>Stora</strong><span className="text-ink-3">Commerce Hub</span></div>
              ...
              <span className="size-8 rounded-full bg-gradient-to-br from-[#82a99e] to-[#229cb1]" aria-hidden />
            </div>
            ...
            <div className="px-3 pb-4 sm:px-6 sm:pb-6 lg:px-10">
              ...
              <div className="grid grid-cols-2 gap-1 py-4 sm:grid-cols-3 lg:grid-cols-5">
                {metrics.map(([label, value], index) => (
                  <div key={label} className={`rounded-[17px] border border-black/[0.04] bg-[#ededed] p-3.5 sm:rounded-[20px] sm:p-4 ${index === 4 ? "hidden sm:block" : ""}`}>
              ...
              <div className="grid gap-2.5 lg:grid-cols-[290px_1fr]">
                <div className="rounded-[20px] bg-[#ededed] p-5 sm:rounded-[22px]">
...
export function HomeLanding() {
  return (
    <>
      ...
      <main id="contenu-principal" className="grain min-h-screen overflow-hidden bg-base text-ink">
      <Navbar />
      ...
      <section id="top" className="relative pb-20 pt-32 sm:pt-40 lg:pt-44">
        ...
        <div className="relative mx-auto max-w-[1440px] px-5 sm:px-8">
          <div className="mx-auto mb-8 flex w-fit items-center gap-2 rounded-full border border-black/[0.07] bg-white/35 px-3.5 py-2 text-[10px] font-medium uppercase tracking-[.16em] text-ink-3 shadow-sm sm:mb-10 sm:text-xs">
            <span className="size-1.5 rounded-full bg-accent shadow-[0_0_0_4px_rgba(31,197,190,.12)]" aria-hidden />
          ...
          <h1 className="mx-auto max-w-[1380px] text-center font-[Urbanist] text-[clamp(1.45rem,6.6vw,7.35rem)] font-light leading-[.9] tracking-[-.075em]">
            ...
            <span className="mt-2 block bg-gradient-to-r from-[#3f6f67] via-[#087a7d] to-[#176480] bg-clip-text text-transparent sm:mt-3">générée par l’IA.</span>
          </h1>
          ...
          <div className="mx-auto mt-9 flex max-w-2xl flex-col items-center gap-7 text-center sm:mt-11">
            <p className="text-base leading-relaxed text-ink-2 sm:text-xl">
              Décrivez votre idée. Stora crée le design, le catalogue, les visuels et les textes pour lancer une boutique prête à vendre.
            </p>
            <div className="flex w-full flex-col justify-center gap-3 sm:w-auto sm:flex-row">
              <a href="/login?mode=signup" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-ink px-6 py-3 text-sm font-medium text-white shadow-[0_12px_28px_-16px_rgba(0,0,0,.55)] transition duration-300 hover:-translate-y-0.5 hover:bg-black/80">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-[11px] text-ink-3">
      ...
      <section aria-label="Les avantages de Stora" className="border-y border-black/[0.08] bg-white/15">
      ...
      <section id="produit" className="scroll-mt-24 py-24 sm:py-32">
        <div className="mx-auto max-w-[1440px] px-5 sm:px-8">
          <div className="grid gap-8 lg:grid-cols-[.7fr_1.3fr] lg:items-end">
            <p className="text-xs font-medium uppercase tracking-[.18em] text-ink-3">Tout ce qu’il faut pour lancer</p>
            <div>
          ...
          <div className="mt-16 grid gap-3 lg:grid-cols-3">
            {benefits.map((benefit, index) => (
              <article key={benefit.title} className="group overflow-hidden rounded-[26px] border border-black/[0.07] bg-[#e9e9e9] p-2 shadow-[0_16px_40px_-34px_rgba(0,0,0,.45)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_24px_50px_-32px_rgba(0,0,0,.4)]">
                <div className="h-[245px] overflow-hidden rounded-[20px] bg-[#dcdedc] sm:h-[280px]">
                  <ProductVisual index={index} />
                </div>
                <div className="p-5 sm:p-6">
                  <p className="text-[10px] font-medium uppercase tracking-[.16em] text-ink-3">{benefit.eyebrow}</p>
                  ...
                  <div className="mt-8 border-t border-black/[0.08] pt-4">
                    ...
                    <span className="ml-2 text-[11px] text-ink-3">{benefit.metricLabel}</span>
                  </div>
                </div>
              </article>
            ))}
      ...
      <section id="methode" className="scroll-mt-24 border-y border-black/[0.08] bg-white/15 py-24 sm:py-32">
        <div className="mx-auto max-w-[1440px] px-5 sm:px-8">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="mx-auto mt-7 max-w-xl text-base leading-relaxed text-ink-2">Vous donnez la direction. Stora exécute et vous gardez le contrôle à chaque étape.</p>
          </div>

          <div className="mt-16 grid overflow-hidden rounded-[28px] border border-black/[0.08] sm:grid-cols-2 lg:grid-cols-4">
            {steps.map((step, index) => (
              <article key={step.n} className={`flex min-h-[330px] flex-col border-black/[0.08] p-7 sm:p-8 ${index === 2 ? "bg-gradient-to-br from-[#3f716a] via-[#087f82] to-[#155f7a] text-white" : "bg-[#e9e9e9]"} [&:not(:last-child)]:border-b sm:[&:nth-child(3)]:border-b-0 sm:[&:nth-child(odd)]:border-r lg:[&:not(:last-child)]:border-r lg:[&:not(:last-child)]:border-b-0`}>
      ...
      <section id="tarifs" className="scroll-mt-24 py-24 sm:py-32">
        <div className="mx-auto max-w-[1440px] px-5 sm:px-8">
          ...
          <div className="mt-16 grid gap-3 lg:grid-cols-3">
            {plans.map((plan) => (
              <article key={plan.name} className={`relative flex min-h-[470px] flex-col overflow-hidden rounded-[28px] p-7 sm:p-9 ${plan.featured ? "bg-gradient-to-br from-[#3f716a] via-[#087f82] to-[#155f7a] text-white shadow-[0_30px_65px_-35px_rgba(22,100,115,.55)]" : "border border-black/[0.06] bg-[#ececec]"}`}>
                {plan.featured && <span className="absolute right-6 top-6 rounded-full bg-white/18 px-3 py-1 text-[10px] font-medium text-white">Le plus choisi</span>}
      ...
      <section id="faq" className="scroll-mt-24 border-t border-black/[0.08] bg-white/15 py-24 sm:py-32">
        <div className="mx-auto grid max-w-[1200px] gap-14 px-5 sm:px-8 lg:grid-cols-[.7fr_1.3fr] lg:gap-20">
          <div>
            ...
            <p className="mt-6 max-w-md text-sm leading-relaxed text-ink-2">Une question reste sans réponse ? Commencez gratuitement et découvrez Stora directement dans l’interface.</p>
          </div>
      ...
      <footer className="mx-auto flex max-w-[1440px] flex-col gap-6 px-6 py-10 text-xs text-ink-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-ink">Stora AI</span>
          <span>© 2026</span>
        </div>
